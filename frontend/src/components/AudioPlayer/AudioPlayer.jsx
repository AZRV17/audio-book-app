...
import { FiPlay, FiPause, FiX, FiVolume2, FiVolume1, FiVolumeX, FiSkipBack, FiSkipForward } from 'react-icons/fi'
    ...
    <div className="fixed bottom-0 left-0 right-0 bg-white border-t border-stone-200 shadow-lg px-4 py-3 z-50">
      ...
      <div className="max-w-4xl mx-auto flex items-center gap-4">
        <div className="flex-shrink-0 min-w-0 w-40">
          ...
          {parts.length > 1 && (
            <p className="text-stone-400 text-xs">Часть {partIndex + 1} / {parts.length}</p>
          )}
        </div>
        ...
        <div className="flex items-center gap-1 flex-shrink-0">
          {parts.length > 1 && (
            <button
              ...
              className="text-stone-400 hover:text-stone-700 disabled:opacity-30 transition-colors cursor-pointer p-1"
            >
              <FiSkipBack size={16} />
            </button>
          )}
