...
export default function BookCard({ book }) {
  return (
    <Link to={`/books/${book.id}`} className="block bg-white border border-stone-200 shadow-md hover:shadow-xl hover:-translate-y-1 rounded-xl overflow-hidden transition-all duration-200">
      {book.cover_url ? (
        <img src={book.cover_url} alt={book.title} className="w-full h-48 object-contain bg-stone-50 p-2" />
      ) : (
        <div className="w-full h-48 bg-amber-50 flex items-center justify-center text-4xl">Ошибка</div>
      )}
      <div className="p-3">
